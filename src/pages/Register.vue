...
const handleRegister = async () => {
  ...
  if (!firstName.value || !lastName.value || !email.value || !phone.value || !password.value) {
    error.value = 'Lütfen tüm zorunlu alanları doldurunuz.'
    return
  }
  
  if (!agree3.value) { 
      error.value = 'Lütfen üyelik sözleşmesini kabul ediniz.'
      return
  }
  ...
  try {
    ...
    console.error(err)
    error.value = err.message || 'Kayıt sırasında hata oluştu.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <section class="auth-page">
    ...
    <div class="auth-panel">
      <div class="auth-tabs">
        <RouterLink to="/login" class="tab" active-class="active">GİRİŞ YAP</RouterLink>
        <RouterLink to="/register" class="tab" active-class="active">ÜYE OL</RouterLink>
      </div>
      ...
      <form class="auth-form" @submit.prevent="handleRegister">
        ...
        <div class="form-row">
          <div class="form-group floating-label-group half-width">
            <input v-model="firstName" type="text" class="custom-input" placeholder=" " required />
            <label class="floating-label">AD*</label>
          </div>
          <div class="form-group floating-label-group half-width">
            <input v-model="lastName" type="text" class="custom-input" placeholder=" " required />
            <label class="floating-label">SOYAD*</label>
          </div>
        </div>

        <div class="form-group floating-label-group">
          <input v-model="email" type="email" class="custom-input" placeholder=" " required />
        ...
        <div class="form-group floating-label-group password-group">
          <input 
            v-model="password" 
...
.auth-form {
  display: flex;
  flex-direction: column;
  gap: 15px; /* مسافة أقل قليلاً بين العناصر */
}
